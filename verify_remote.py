...
SPACE_URL = os.environ.get(
    "SPACE_URL", "https://rishi-harti768-k8s-cost-optimizer.hf.space"
).rstrip("/")
...
def test_remote_server():
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            log_test_step(
                "Health Check", False, f"HTTP {resp.status_code} - {resp.text}"
            )
            return
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            log_test_step(
                "Reset Env", True, f"Episode ID: {data.get('episode_id', 'N/A')}"
            )
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            log_test_step(
                "Get State",
                True,
                f"Step: {data.get('step_count', 0)}, Replicas: {data.get('replicas', 'N/A')}",
            )
        else:
    ...
    try:
        ...
        if resp.status_code == 200:
            result = resp.json()
            log_test_step(
                "Step Execution", True, f"Reward: {result.get('reward', 0.0):.2f}"
            )
        else:
            log_test_step(
                "Step Execution", False, f"HTTP {resp.status_code} - {resp.text}"
            )
