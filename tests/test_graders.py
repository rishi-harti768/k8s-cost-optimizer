...
class TestObservationMetrics:
    ...
    def test_is_healthy_uptime(self):
        assert is_healthy_uptime(280.0)
        assert not is_healthy_uptime(300.0)
        assert not is_healthy_uptime(350.0)

    def test_is_warning_zone(self):
        assert is_warning_zone(200.0)
        assert is_warning_zone(250.0)
        assert is_warning_zone(299.9)
        assert not is_warning_zone(300.0)
        assert not is_warning_zone(150.0)

    def test_steal_violation(self):
        assert steal_violation(0.20)
        assert steal_violation(0.25)
        assert not steal_violation(0.19)
        assert not steal_violation(0.0)
...
class TestColdStartGrader:
    ...
    def test_all_errors(self, dummy_observation):
        ...
        obs_with_errors = Observation(
            ...
            mem_usage_pct=50.0,
            p99_latency_ms=180.0,
            http_error_rate=1.0,  # 100% errors
            cpu_steal_pct=0.05,
            active_replicas=3,
            buffer_depth=5,
            node_size_class=NodeSizeClass.MEDIUM,
    ...
    def test_partial_errors(self, dummy_observation):
        ...
        step_no_err = TrajectoryStep(
            observation=obs_no_err,
            action=ActionType.MAINTAIN,
            reward=1.0,
            done=False,
            info={},
        )
        step_with_err = TrajectoryStep(
            observation=obs_with_err,
            action=ActionType.MAINTAIN,
            reward=1.0,
            done=False,
            info={},
...
class TestEfficientSqueezeGrader:
    ...
    def test_all_violations(self, dummy_observation):
        ...
        obs_violation = Observation(
            ...
            p99_latency_ms=180.0,
            http_error_rate=0.0,
            cpu_steal_pct=0.25,  # Violation: >= 0.20
            active_replicas=3,
            buffer_depth=5,
            node_size_class=NodeSizeClass.MEDIUM,
            current_hourly_cost=40.0,
...
class TestEntropyStormGrader:
    ...
    def test_violation_with_proactive_rebalance(self, dummy_step):
        ...
        obs_rebalance = Observation(
            ...
            p99_latency_ms=180.0,
            http_error_rate=0.0,
            cpu_steal_pct=0.12,  # Rising steal signal
            active_replicas=3,
            buffer_depth=5,
            node_size_class=NodeSizeClass.MEDIUM,
            current_hourly_cost=40.0,
